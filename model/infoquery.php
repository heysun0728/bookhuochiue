...
<?php 
	...
	if ($_SERVER["REQUEST_METHOD"] == "GET") {
	  	$con=test_input($_GET['inputcondition']);
	}else if ($_SERVER["REQUEST_METHOD"] == "POST") {
	  	$con=test_input($_POST['inputcondition']);
	}
    ...
    $sql='SELECT m.vNumber,m.ID,m.Name,m.ApplyState,s.schoolName,m.ServiceHours
	      ...
	      WHERE m.ID LIKE :id OR m.Name LIKE :name OR s.schoolName LIKE :school
	      ORDER BY m.vNumber';
	
	//prepare() 預處理操作
	$rs=$link->prepare($sql);
	$rs->bindValue(':id','%'.$con.'%');
	$rs->bindValue(':name','%'.$con.'%');
	$rs->bindValue(':school','%'.$con.'%');
